...
import 'zoomable_header_widget.dart';
...
class DayView extends ZoomableHeadersWidget<DayViewStyle, DayViewController> {
  ...
  DayView({
    ...
    DayBarTapCallback? onDayBarTappedDown,
        ...
        date = date,
        dayBarStyle =
            dayBarStyle ?? DayBarStyle.fromDate(date: date.date.yearMonthDay),
        super(
          style: style ?? DayViewStyle.fromDate(date: date.date.yearMonthDay),
          hoursColumnStyle: hoursColumnStyle ??
          ...
          initialTime: initialTime?.atDate(date.date.yearMonthDay) ??
              (Utils.sameDay(date.date.yearMonthDay)
                      ? HourMinute.now()
                      : const HourMinute())
                  .atDate(date.date.yearMonthDay),
...
class _DayViewState extends ZoomableHeadersWidgetState<DayView> {
  ...
  Widget createMainWidget() {
    ...
    if (Utils.sameDay(widget.date.date) &&
        widget.minimumTime.atDate(widget.date.date).isBefore(DateTime.now()) &&
        widget.maximumTime.atDate(widget.date.date).isAfter(DateTime.now())) {
